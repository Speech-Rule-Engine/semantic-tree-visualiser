...
streeVis.translateMathML = function() {
  try {
    streeVis.show(MathJax.mathml2mml(
      sre.Enrich.prepareMmlString(window.input.value)));
  } catch (e) {
    console.error(e);
...
streeVis.translateSRE = function() {
  try {
    streeVis.show(sre.Enrich.prepareMmlString(
      sre.DomUtil.formatXml(window.input.value).trim()));
  } catch (e) {
    console.error(e);
    alert('Incorrect MathML!');
    return;
...
streeVis.showMathml = function (mathml) {
  ...
  if (window.showMathml.checked) {
    mathml = mathml.replace(/data-semantic-/g, '');
...
streeVis.showSpeech = function(mathml) {
  ...
  streeVis.addText(div1, SRE.toSpeech(mathml));
  SRE.setupEngine({domain: 'clearspeak', style: 'default'});
  let div2 = document.createElement('div');
  streeVis.addText(div2, SRE.toSpeech(mathml));
