 ...
 * @constructor
 */
streeVis = function(json, node = d3.select('#body').node()) {
  this.json = json;
...
streeVis.prototype.orientation = function(maxWidth = window.innerWidth,
  ...
  if (header) {
    maxHeight = maxHeight - header.clientHeight / 2;
  }

  if (streeVis.config.tests) {
    let depth = getDepth(this.json);
    maxHeight = depth * 70 + 70;
  }
...
streeVis.run = function(option) {
  ...
  streeVis.currentSVG = new streeVis(streeVis.config.json);
...
streeVis.show = async function(mml) {
  if (!mml) return;
  if (streeVis.config.nemeth) {
    streeVis.showNemeth(mml);
    return;
  }
...
streeVis.setLocale = function() {
  ...
  if (SRE.variables?.LOCALES) {
    for (let [iso, lang] of SRE.variables.LOCALES.entries()) {
...
streeVis.getHover = function(data) {
  if (streeVis.config.trie || streeVis.config.tests) {
...
streeVis.renderCells = function() {
  streeVis.config.tests = true;
  streeVis.config.scale *= streeVis.config.testsScale;
  let cells = document.querySelectorAll('.cell');
  for (let cell of cells) {
    let math = cell.querySelector('.math').querySelector('math');
    let tree = cell.querySelector('.tree');
    let vistree = new streeVis(SRE.toJson(math.outerHTML), tree);
    vistree.init();
  }
};


streeVis.nemeth = function() {
  if (window.location.search.length > 1) {
    let value = unescape(window.location.search.replace(/.*\?/,''));
    window.input.value = value;
  }
  SRE.setupEngine(
    {locale: 'nemeth', modality: 'braille', domain: 'default',
     markup: 'layout'
    });
};


streeVis.setBreakValues = function(max) {
  let element = document.getElementById('break');
  let set = parseInt(element.value);
  element.innerHTML = '';
  for (var i = 0; i <= max; i++) {
    let option = document.createElement('option');
...
streeVis.keepNemeth = function() {
  window.location =
    String(window.location).replace(/\?.*/,'') + '?' +
    escape(window.input.value);
};

streeVis.nemethExpr = null;

streeVis.showNemeth = function(mml) {
  ...
  for (const line of speech.split(/<|>/)) {
    if (line.match(/^br/)) {
      const value = parseInt(line.match(/\d+/));
      maxCount = Math.max(value, maxCount);
      streeVis.nemethExpr.push(
        [value, document.createElement('br')]);
    } else {
      streeVis.nemethExpr.push([0, document.createTextNode(line)]);
    }
  }
  streeVis.setBreakValues(maxCount);
  streeVis.displayNemeth();
};

streeVis.displayNemeth = function() {
  const element = document.getElementById('body');
  element.innerHTML = '';
  const div = document.createElement('div');
  div.style.fontSize = '20pt';
  element.appendChild(div);
  if (streeVis.nemethExpr) {
    let set = parseInt(document.getElementById('break').value);
    for (const [value, line] of streeVis.nemethExpr) {
      if (value <= set) {
