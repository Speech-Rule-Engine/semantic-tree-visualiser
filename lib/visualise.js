...
streeVis.rerender = function() {
  ...
  new Promise((ok, fail) => {
    setTimeout(function execute() {
      ...
      return (MathJax.tex2mml && MathJax.mathml2mml && sre.System &&
...
streeVis.showEnrMml = function (mathml) {
  window.enrmml.innerHTML = '';
  if (window.showEnrMml.checked) {
    let mml = sre.System.getInstance().toEnriched(mathml);
    let xs = new XMLSerializer();
    let emml = xs.serializeToString(mml);
...
streeVis.showSemantics = function (mathml) {
  window.semantics.innerHTML = '';
  if (window.showSemantics.checked) {
    let stree = sre.System.getInstance().toSemantic(mathml);
    let xs = new XMLSerializer();
